...
object SupervisorStrategy extends SupervisorStrategyLowPriorityImplicits {
  ...
  /**
    ...
    * the given Throwables matches the cause and restarts, otherwise escalates.
    */
  def makeDecider(trapExit: JIterable[Class[_ <: Throwable]]): Decider = makeDecider(trapExit)
...
abstract class SupervisorStrategy {
    ...
    if (loggingEnabled) {
      ...
      decision match {
        case Resume   ⇒
         ...
         new ActorLoggingAdapter(child, context.system).warning(logMessage)
        case Escalate ⇒ // don't log here
        case _        ⇒
          //publish(context, Error(cause, child.path.toString, getClass, logMessage))
          new ActorLoggingAdapter(child, context.system).error(cause, logMessage)
      }
    }
  ...
  /**
    ...
    */
  final def resumeChild(child: ActorRef, cause: Throwable): Unit = child.asInstanceOf[LocalActorRef].resume(causedByFailure = cause)
  ...
  final def restartChild(child: ActorRef, cause: Throwable, suspendFirst: Boolean): Unit = {
    val c = child.asInstanceOf[LocalActorRef]
    if (suspendFirst) c.suspend()
    c.cell.restart(cause)
  }
